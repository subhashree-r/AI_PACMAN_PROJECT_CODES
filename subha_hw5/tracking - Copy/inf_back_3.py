    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particleList:
            temp = list(oldParticle) # A list of ghost positions
            ...
            for i in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, temp),i, self.ghostAgents[i])
                temp[i] = util.sample(newPosDist)
            "*** END YOUR CODE HERE ***"
