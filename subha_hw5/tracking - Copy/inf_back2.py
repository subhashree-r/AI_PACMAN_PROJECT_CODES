...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allpossible = util.Counter()
        for pos in self.legalPositions:
            dist = self.getPositionDistribution(self.setGhostPosition(gameState, pos)) #distance gives a set of distribution and probability to the ghost's possible next state assuming the ghost in game state
            for nextpos, p in dist.items():
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        avg = self.numParticles / len(positions)
        while c < avg:
    ...
    def elapseTime(self, gameState):
        ...
        my_dist = []
        for pos in self.particles:
            act_dist=self.getBeliefDistribution(self.setGhostPosition(gameState,pos))#updating the distribution based on previous position of pacman and game state
            my_dist.append(util.sample(act_dist))


        #     my_dist=util.nSample(dist.values(),dist.keys(),self.numParticles)
        self.particles = my_dist
        #dist=self.getPositionDistribution(self.set)
    ...
    def getBeliefDistribution(self):
        ...
        dist = util.Counter()
        for pos in self.particles:
            dist[pos] += 1 #we assign a distribution of 1 for all positions of particles
        dist.normalize()
        return dist
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        states = len(permutations)
        c=0
        temp=[];
        if (states > self.numParticles):
             for i in range(self.numParticles):
                 temp.append(permutations[i])
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for i in range(self.numGhosts):
            if (noisyDistances[i] == None):
                ...
                for p in self.particleList:
                    newList.append(self.getParticleWithGhostInJail(p, i))
        ...
        for par in self.particleList:
            ...
            for i in range(self.numGhosts):
                ...
                tDist = util.manhattanDistance(par[i], pacmanPosition)
                prob = prob * emissionModels[i][tDist]
            Counter[par] += prob
        ...
        newList = []
        for i in range(self.numParticles):
            newList.append(util.sample(Counter))
        self.particleList = newList
        return
