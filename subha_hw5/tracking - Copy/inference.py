...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None: # This shows that the ghost has been captured and hence the belief at jail positions is set to 1
            allPossible[self.getJailPosition()] = 1.0
        else:
            for pos in self.legalPositions:
                truedistance = util.manhattanDistance(pos, pacmanPosition)
                #we do belief update according to equation w = w * P(xi | Parents(Xi))
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allpossible = util.Counter()
        for pos in self.legalPositions:
            distance = self.getPositionDistribution(self.setGhostPosition(gameState, pos)) #distance gives a set of distances and probability to the ghost's possible next state assuming the ghost in game state
            for nextpos, p in distance.items():
                allpossible[nextpos] += self.beliefs[pos] * p
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while c < (self.numParticles / len(positions)):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            my_dist = util.Counter()
            for pos in self.particles:
                dist=util.manhattanDistance(pacmanPosition, pos)
                my_dist[pos] += emissionModel[dist]
            if my_dist.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                self.particles = util.nSample(my_dist.values(),my_dist.keys(),self.numParticles)
...
class JointParticleFilter:
	...
	c = 0
	self.particles = []
	#permutation = permutations(lp, 2)
	#print permutation , "permutationnnnnnnnnnn"
        permutation = list(itertools.product(lpos, repeat = n)) #combinatoric generator
	...
	for p in range(len(self.particles)):
		...
		for i in range(ghosts):
			if noisyDistances[i] == None: # a case where ghost is captured by pacman
                    		particle = self.getParticleWithGhostInJail(self.particles[p], i)
                	else:
                    		distance = util.manhattanDistance(self.particles[p][i], pacmanPosition)
				em = emissionModels[i][distance]
                    		val *= em
                allPossible[particle] += val


	flag = 1
  	for a in allPossible.values():
		if (a):
			#print "kkkkkkkkkkk"
                	flag=0
			break
		...
		summation = summation + a
            ...
            if summation == 0: return			#normalising the values to make the total sum =1
            for key in allPossible.keys():
            	allPossible[key] = allPossible[key] / summation
            self.particles = []
    ...
    def elapseTime(self, gameState):
	    ...
	    ghosts = self.numGhosts
	    for i in range(ghosts):
			ga = self.ghostAgents[i]
			gp = setGhostPositions(gameState, newParticle) #alters the gamestate - the ghosts are places in the supplied position
                	newPosDist = getPositionDistributionForGhost(gp,i,ga )
	...
	for p in particles:
        ...
        summation = 0.0
	for a in beliefdist.values():
		summation = summation + a

        if summation == 0: return			#normalising the values to make the total sum =1
        for key in beliefdist.keys():
            beliefdist[key] = beliefdist[key] / summation
