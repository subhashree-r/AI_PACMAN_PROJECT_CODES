...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        emissionModel = busters.getObservationDistribution(noisyDistance)
        pacmanPosition = gameState.getPacmanPosition()
        l1=xrange(self.numParticles)
        if noisyDistance == None:
            self.particles = [self.getJailPosition() for i in l1]
        else:
            weights = util.Counter()
            for pos in self.particles:
                dist=util.manhattanDistance(pacmanPosition, pos)
                weights[pos] += emissionModel[dist]
            if weights.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                self.particles = [util.sample(weights) for i in l1]
    ...
    def initializeParticles(self):
        ...
        if (states > self.numParticles):
            for i in range(self.numParticles):
                n.append(permutations[i])
            self.particleList = n
            return
        else:
            partPos = self.numParticles/states
            c = 0
            newList = []

            for p in permutations:
                for i in range(partPos):
                    newList.append(p)
                    c += 1
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particleList:
            ...
            "*** YOUR CODE HERE ***"
            "Note Even though i was getting Correct answer there was a timeout error." \
            "For rectifying this I have referenced online for Optimization"
            for i in range(self.numGhosts):
